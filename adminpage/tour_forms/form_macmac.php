...
$touryear = null;
$touryear_query = "SELECT year FROM tours WHERE tourname = '$tourname' LIMIT 1";
$touryear_result = mysqli_query($con, $touryear_query);
if ($touryear_result && mysqli_num_rows($touryear_result) > 0) {
    $touryear_data = mysqli_fetch_assoc($touryear_result);
    $touryear = $touryear_data['year'];
} else {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function isFieldConditionallyRequired(field) {
                ...
                const dependentStatus = form.querySelector('input[name="dependent_status"]:checked');
                ...
                switch(status) {
                    case 'employee':
                        if (document.getElementById('employeeSection').style.display === 'block') {
                            return ['employment_letter', 'pay_slips'].includes(fieldName);
                        }
                        break;
                    case 'business':
                        if (document.getElementById('businessSection').style.display === 'block') {
                            return ['business_registration'].includes(fieldName);
                        }
                        break;
                    case 'freelancer':
                        if (document.getElementById('freelancerSection').style.display === 'block') {
                            return ['service_letters'].includes(fieldName);
                        }
                        break;
                    case 'student':
                        if (document.getElementById('studentDependentSection').style.display === 'block') {
                            if (depStatus === 'student') {
                                return ['dependent_status', 'student_letter'].includes(fieldName);
                            } else if (depStatus === 'dependent') {
                                return ['dependent_status', 'dependentConfirmationDependent'].includes(fieldName);
                            }
                        }
                        break;
                }
                return fieldName === 'dependent_status';
            }
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (!validateRadioGroup('employment_status')) {
                    isValid = false;
                ...
                if (employmentStatus && employmentStatus.value === 'student') {
                    if (!validateRadioGroup('dependent_status')) {
                        isValid = false;
            ...
            <?php if ($existingdata && $existingdata['employment_status']): ?>
                showEmploymentSection('<?php echo $existingdata['employment_status']; ?>');
            <?php endif; ?>

            <?php if ($existingdata && $existingdata['dependent_status']): ?>
                showDependentSection('<?php echo $existingdata['dependent_status']; ?>');
            <?php endif; ?>
        });
    </script>
</body>
