...
if (isset($_POST['search_nic']) && !empty($_POST['search_nic'])) {
    ...
    $query = "SELECT * FROM visa_applications WHERE nic_number = '$nic_search'";
    ...
    $query = "SELECT * FROM visa_applications WHERE nic_number = '$nic'";
    $result = mysqli_query($con, $query);
    if ($result && mysqli_num_rows($result) > 0) {
        $existing_data = mysqli_fetch_assoc($result);
        $editing = true;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['search_nic'])) {
    ...
    try {
        ...
        function handleFileUpload($file_input_name, $surname, $othername, $file_prefix) {
            global $upload_dir;

            if (isset($_FILES[$file_input_name]) && $_FILES[$file_input_name]['error'] == 0) {
                // Set folder: upload/visa/surname_othername/
                $folder_name = trim($surname . '_' . $othername);
                $folder_name = preg_replace('/[^a-zA-Z0-9_-]/', '_', $folder_name);
                $user_folder = $upload_dir . 'visa/' . $folder_name . '/';
                ...
                if (move_uploaded_file($file_tmp, $upload_path)) {
                    return 'visa/' . $folder_name . '/' . $new_filename;
                } else {
        ...
        function handleMultipleFileUploads($file_input_name, $surname, $othername, $file_prefix) {
            global $upload_dir;
            ...
            if (isset($_FILES[$file_input_name]) && is_array($_FILES[$file_input_name]['name'])) {
                ...
                for ($i = 0; $i < count($_FILES[$file_input_name]['name']); $i++) {
                    if ($_FILES[$file_input_name]['error'][$i] == 0) {
                        ...
                        if (move_uploaded_file($file_tmp, $upload_path)) {
                            $uploaded_files[] = 'visa/' . $folder_name . '/' . $new_filename;
                        }
                    }
                }
            }
        ...
        $check_query = "SELECT * FROM visa_applications WHERE nic_number = '$nic_number'";
        ...
        // Handle file uploads
        $passport_copy = handleFileUpload('passportCopy', $surname, $othername, 'passport_copy');
        $photo_id = handleFileUpload('photoId', $surname, $othername, 'photo_id');
        $visa_request_letter = handleFileUpload('visaRequestLetter', $surname, $othername, 'visa_request_letter');
        $bank_statements = handleFileUpload('bankStatements', $surname, $othername, 'bank_statements');
        $employment_letter = handleFileUpload('employmentLetter', $surname, $othername, 'employment_letter');
        $epf_confirmation = handleFileUpload('epfConfirmation', $surname, $othername, 'epf_confirmation');
        $pay_slips = handleMultipleFileUploads('paySlips', $surname, $othername, 'pay_slip');
        $business_registration = handleFileUpload('businessRegistration', $surname, $othername, 'business_registration');
        $form_pvt_ltd = handleFileUpload('formPvtLtd', $surname, $othername, 'form_pvt_ltd');
        $company_statements = handleMultipleFileUploads('companyStatements', $surname, $othername, 'company_statement');
        $service_letters = handleMultipleFileUploads('serviceLetters', $surname, $othername, 'service_letter');
        $student_letter = handleFileUpload('studentLetter', $surname, $othername, 'student_letter');
        $dependent_confirmation = handleFileUpload('dependentConfirmation', $surname, $othername, 'dependent_confirmation');
        $dependent_income = handleFileUpload('dependentIncome', $surname, $othername, 'dependent_income');
        $other_documents = handleMultipleFileUploads('otherDocuments', $surname, $othername, 'other_document');
        ...
        if ($record_exists) {
            ...
            // File updates - only update if new files are uploaded, preserve existing
            ...
            if (!empty($update_parts)) {
                $update_query = "UPDATE visa_applications SET " . implode(', ', $update_parts) . " WHERE nic_number = '$nic_number'";
    ...
    if ($response['success']) {
        ...
        $query = "SELECT * FROM visa_applications WHERE nic_number = '$nic_number'";
        $result = mysqli_query($con, $query);
        if ($result && mysqli_num_rows($result) > 0) {
            $existing_data = mysqli_fetch_assoc($result);
            $editing = true;
...
<head>
    ...
    <title>Visa & Airticket Document Submission</title>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // [All other JS setup code remains unchanged]

    // --- MODIFIED SUBMISSION HANDLING BELOW THIS LINE ONLY ---
    form.addEventListener('submit', function(e) {
        ...
        .then(data => {
            // If update is successful, show a popup and reset form
            if (data.success) {
                alert('✅ ' + data.message);
                // Clear editing state by reloading the page without parameters
                window.location.href = window.location.pathname; // Reset form fields

                // Optionally: Reset UI states such as valid/locked styling
                const allFields = form.querySelectorAll('input, select, textarea');
                allFields.forEach(field => {
                    field.classList.remove('valid', 'error', 'locked');
                    field.readOnly = false;
                    field.disabled = false;
                });
                // Optionally clear all uploaded file name displays and uploaded classes
                const uploadDisplays = form.querySelectorAll('.file-name-display');
                uploadDisplays.forEach(div => { div.innerHTML = ''; });
                const uploadBoxes = form.querySelectorAll('.file-upload');
                uploadBoxes.forEach(div => { div.classList.remove('uploaded'); });
    ...
    // [All the rest of your JS - form field validation, UX, etc. - remains unchanged]
});
</script>
            ...
            <div class="nic-search-section">
                ...
                <?php if (isset($_POST['search_nic']) && !empty($_POST['search_nic']) && !$existing_data): ?>
                    <div class="alert alert-info" style="margin-top: 15px;">
                        ℹ️ No existing record found for NIC: <?php echo htmlspecialchars($_POST['search_nic']); ?>. You can create a new application.
                    </div>
                <?php endif; ?>
            </div>
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <div class="file-upload <?php echo ($existing_data && $existing_data['visa_request_letter']) ? 'uploaded' : ''; ?>">
                    ...
                    <p>Drag and drop your file here or click to browse</p>
                    <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                    <input type="file" id="visaRequestLetter" name="visaRequestLetter" accept=".jpg,.jpeg,.pdf">
                    <div class="file-name-display" id="visaRequestLetterDisplay">
                        <?php if ($existing_data && $existing_data['visa_request_letter']): ?>
                            ✅ Current file: <?php echo htmlspecialchars($existing_data['visa_request_letter']); ?>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="file-upload <?php echo ($existing_data && $existing_data['bank_statements']) ? 'uploaded' : ''; ?>">
                    ...
                    <p>Drag and drop your file here or click to browse</p>
                    <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                    <input type="file" id="bankStatements" name="bankStatements" accept=".jpg,.jpeg,.pdf">
                    <div class="file-name-display" id="bankStatementsDisplay">
                        <?php if ($existing_data && $existing_data['bank_statements']): ?>
                            ✅ Current file: <?php echo htmlspecialchars($existing_data['bank_statements']); ?>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                    ...
                    <div class="radio-group">
                        <label>
                            <input type="radio" name="employmentStatus" value="employee" onclick="showEmploymentSection('employee')" 
                                   ...
                                   <?php echo shouldLockField($existing_data, 'employment_status') ? 'disabled' : ''; ?>>
                            <span>👨‍💼 Employee</span>
                        </label>
                        <label>
                ...
                <div id="employeeSection" class="employment-section" style="display: <?php echo ($existing_data && $existing_data['employment_status'] == 'employee') ? 'block' : 'none'; ?>;">
                    <div class="file-upload" style="margin-bottom: 20px;">
                        ...
                        <p>Drag and drop your file here or click to browse</p>
                        <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                        <input type="file" id="employmentLetter" name="employmentLetter" accept=".jpg,.jpeg,.pdf">
                        <div class="file-name-display" id="employmentLetterDisplay">
                            <?php if ($existing_data && $existing_data['employment_letter']): ?>
                                ✅ Current file: <?php echo htmlspecialchars($existing_data['employment_letter']); ?>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="file-upload" style="margin-bottom: 20px;">
                        ...
                        <p>Drag and drop your file here or click to browse</p>
                        <p style="font-size: 14px;                        color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                        <input type="file" id="epfConfirmation" name="epfConfirmation" accept=".jpg,.jpeg,.pdf">
                        <div class="file-name-display" id="epfConfirmationDisplay">
                            <?php if ($existing_data && $existing_data['epf_confirmation']): ?>
                                ✅ Current file: <?php echo htmlspecialchars($existing_data['epf_confirmation']); ?>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <div id="businessSection" class="employment-section" style="display: <?php echo ($existing_data && $existing_data['employment_status'] == 'business') ? 'block' : 'none'; ?>;">
                    <div class="file-upload" style="margin-bottom: 20px;">
                        ...
                        <p>Drag and drop your file here or click to browse</p>
                        <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                        <input type="file" id="businessRegistration" name="businessRegistration" accept=".jpg,.jpeg,.pdf">
                        <div class="file-name-display" id="businessRegistrationDisplay">
                            <?php if ($existing_data && $existing_data['business_registration']): ?>
                                ✅ Current file: <?php echo htmlspecialchars($existing_data['business_registration']); ?>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="file-upload" style="margin-bottom: 20px;">
                        ...
                        <p>Drag and drop your file here or click to browse</p>
                        <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                        <input type="file" id="formPvtLtd" name="formPvtLtd" accept=".jpg,.jpeg,.pdf">
                        <div class="file-name-display" id="formPvtLtdDisplay">
                            <?php if ($existing_data && $existing_data['form_pvt_ltd']): ?>
                                ✅ Current file: <?php echo htmlspecialchars($existing_data['form_pvt_ltd']); ?>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <div id="studentDependentSection" class="employment-section" style="display: <?php echo ($existing_data && $existing_data['employment_status'] == 'student') ? 'block' : 'none'; ?>;">
                        ...
                        <div class="radio-group">
                            <label>
                                <input type="radio" name="dependentStatus" value="student" onclick="showDependentSection('student')"
                                       ...
                                       <?php echo shouldLockField($existing_data, 'dependent_status') ? 'disabled' : ''; ?>>
                                <span>🎓 Student</span>
                            </label>
                            <label>
                    ...
                    <div id="studentSubSection" style="display: <?php echo ($existing_data && $existing_data['dependent_status'] == 'student') ? 'block' : 'none'; ?>;">
                        <div class="file-upload" style="margin-bottom: 20px;">
                            ...
                            <p>Drag and drop your file here or click to browse</p>
                            <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                            <input type="file" id="studentLetter" name="studentLetter" accept=".jpg,.jpeg,.pdf">
                            <div class="file-name-display" id="studentLetterDisplay">
                                <?php if ($existing_data && $existing_data['student_letter']): ?>
                                    ✅ Current file: <?php echo htmlspecialchars($existing_data['student_letter']); ?>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="file-upload" style="margin-bottom: 20px;">
                            ...
                            <p>Drag and drop your file here or click to browse</p>
                            <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                            <input type="file" id="dependentConfirmationStudent" name="dependentConfirmation" accept=".jpg,.jpeg,.pdf">
                            <div class="file-name-display" id="dependentConfirmationStudentDisplay">
                                <?php if ($existing_data && $existing_data['dependent_confirmation']): ?>
                                    ✅ Current file: <?php echo htmlspecialchars($existing_data['dependent_confirmation']); ?>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="file-upload" style="margin-bottom: 20px;">
                            ...
                            <p>Drag and drop your file here or click to browse</p>
                            <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                            <input type="file" id="dependentIncomeStudent" name="dependentIncome" accept=".jpg,.jpeg,.pdf">
                            <div class="file-name-display" id="dependentIncomeStudentDisplay">
                                <?php if ($existing_data && $existing_data['dependent_income']): ?>
                                    ✅ Current file: <?php echo htmlspecialchars($existing_data['dependent_income']); ?>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <div id="dependentSubSection" style="display: <?php echo ($existing_data && $existing_data['dependent_status'] == 'dependent') ? 'block' : 'none'; ?>;">
                        <div class="file-upload" style="margin-bottom: 20px;">
                            ...
                            <p>Drag and drop your file here or click to browse</p>
                            <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                            <input type="file" id="dependentConfirmationDependent" name="dependentConfirmation" accept=".jpg,.jpeg,.pdf">
                        ...
                        <div class="file-upload" style="margin-bottom: 20px;">
                            ...
                            <p>Drag and drop your file here or click to browse</p>
                            <p style="font-size: 14px; color: var(--dark-gray);">Accepted formats: JPG, PDF (Max size: 2MB)</p>
                            <input type="file" id="dependentIncomeDependent" name="dependentIncome" accept=".jpg,.jpeg,.pdf">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Field validation and green highlighting
    ...
    window.showEmploymentSection = function(status) {
        ...
        document.getElementById('studentDependentSection').style.display = 'none';
    ...
    window.showDependentSection = function(status) {
        document.getElementById('studentSubSection').style.display = 'none';
        document.getElementById('dependentSubSection').style.display = 'none';
        
        if (status === 'student') {
            document.getElementById('studentSubSection').style.display = 'block';
        } else if (status === 'dependent') {
            document.getElementById('dependentSubSection').style.display = 'block';
        }
    };
